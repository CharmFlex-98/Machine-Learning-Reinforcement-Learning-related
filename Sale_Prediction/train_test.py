...
train = True
train_path = r'C:\Users\ASUS\PycharmProjects\Pytorch\Sale_Prediction\train_v9rqX0R.csv'
test_path = r'C:\Users\ASUS\PycharmProjects\Pytorch\Sale_Prediction\test_AbJTz2l.csv'
weight_path = r'C:\Users\ASUS\PycharmProjects\Pytorch\Sale_Prediction\best_weight.pt'
write_path = r'C:\Users\ASUS\PycharmProjects\Pytorch\Sale_Prediction\submission.csv'
figure_path = r'C:\Users\ASUS\PycharmProjects\Pytorch\Sale_Prediction\graph.jpg'
batch_size = 128
learning_rate = 0.001
epoch = 200
input_layer = 44
...
def preprocess_data(data, model_data=None):
    if train:
        data = data.drop(columns=['Item_Identifier', 'Item_Outlet_Sales', 'Outlet_Identifier'])
        ...
        data['Outlet_Establishment_Year'] = data['Outlet_Establishment_Year'].astype('str')
        model_data = model_data.drop(columns=['Item_Identifier', 'Item_Outlet_Sales', 'Outlet_Identifier'])
        model_data['Outlet_Establishment_Year'] = model_data['Outlet_Establishment_Year'].astype('str')
...
def write_result(_prediction, _id, _file):
    ...
    _id1 = _id[0][0]
    _id2 = _id[1][0]
    _file.writelines('{},{},{}{}'.format(_id1, _id2, _prediction, '\n'))
...
class my_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if train:
            label = self.data['Item_Outlet_Sales'][index]

            return variable, label
        else:
            ID = self.data['Item_Identifier'][index]
            ID2 = self.data['Outlet_Identifier'][index]

            return variable, ID, ID2
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.75, patience=int(epoch / 8), verbose=True)
...
if train:
    ...
    with open(write_path, 'w') as file:
        file.writelines('{},{},{}{}'.format('Item_Identifier', 'Outlet_Identifier', 'Item_Outlet_Sales', '\n'))
        with torch.no_grad():
            for batch in test_loader:
                variable, ID, ID2 = batch
                variable = variable.to(device)
                prediction = model(variable)
                write_result(prediction, [ID, ID2], file)
